...
public static class MetadataRepair
{
    ...
    public sealed record CapturedLayout(
        int TypeDefinitionCount,
        int MethodDefinitionCount,
        int FieldCount,
        int[] RenameRows);

    public static readonly CapturedLayout V0173Layout =
        new(4035, 22570, 14625, new[] { 0x322, 0x3c3, 0x3e3 });
    ...
    public static Result Prepare(byte[] input, CapturedLayout? requiredLayout)
    {
        ...
        string Identity(TypeDefinitionHandle handle)
        {
            ...
            return MetadataTokens.GetRowNumber(t.GetDeclaringType()) + ":" +
                   md.GetString(t.Namespace) + ":" +
                   md.GetString(t.Name);
        }
        ...
        if (groups.Length != 2 ||
            !groups.Any(g => g.Key == "0::<>O" && g.Count() == 3) ||
            !groups.Any(g => g.Key == "0::<>c" && g.Count() == 2))
        {
            throw new InvalidDataException("Unrecognized duplicate layout; no changes were made.");
        }

        if (requiredLayout is not null &&
            (all.Length != requiredLayout.TypeDefinitionCount ||
             md.GetTableRowCount(TableIndex.MethodDef) != requiredLayout.MethodDefinitionCount ||
             md.GetTableRowCount(TableIndex.Field) != requiredLayout.FieldCount))
        {
        ...
        foreach (var group in groups)
        {
            ...
            foreach (var handle in group.OrderBy(h => MetadataTokens.GetRowNumber(h)))
            {
                ...
                if (!t.GetDeclaringType().IsNil ||
                    md.GetString(t.Namespace) != "" ||
                    t.GetMethods().Count != 0 ||
                    t.GetFields().Count != 0 ||
                    t.GetProperties().Count != 0 ||
                    t.GetEvents().Count != 0 ||
                    t.GetNestedTypes().Length != 0 ||
                    t.GetGenericParameters().Count != 0)
                {
                    throw new InvalidDataException(
                        "A duplicate is not an empty top-level helper; no changes were made.");
                }
                ...
                if (md.GetString(MetadataTokens.StringHandle(newIndex)) != expected ||
                    !names.Add("0::" + expected))
                {
                    throw new InvalidDataException(
                        "Replacement name would collide; no changes were made.");
                }

                int row = MetadataTokens.GetRowNumber(handle);
                int offset = checked(tableOffset + (row - 1) * rowSize + 4);
                var oldBytes = input.AsSpan(offset, indexSize);
                int stored = indexSize == 4
                    ? checked((int)BinaryPrimitives.ReadUInt32LittleEndian(oldBytes))
                    : BinaryPrimitives.ReadUInt16LittleEndian(oldBytes);

                if (stored != oldIndex)
                    throw new InvalidDataException("Unexpected metadata offset.");
        ...
        if (requiredLayout is not null &&
            !plan.Select(p => p.Row)
                .OrderBy(x => x)
                .SequenceEqual(requiredLayout.RenameRows.OrderBy(x => x)))
        {
        ...
        foreach (var p in plan)
        {
            if (indexSize == 4)
                BinaryPrimitives.WriteUInt32LittleEndian(
                    output.AsSpan(p.Offset, indexSize),
                    checked((uint)p.NewIndex));
            else
                BinaryPrimitives.WriteUInt16LittleEndian(
                    output.AsSpan(p.Offset, indexSize),
                    checked((ushort)p.NewIndex));
        }
        ...
        var afterIds = after.TypeDefinitions.Select(h =>
        {
            ...
            return MetadataTokens.GetRowNumber(t.GetDeclaringType()) + ":" +
                   after.GetString(t.Namespace) + ":" +
                   after.GetString(t.Name);
        }).ToArray();

        if (afterIds.Distinct(StringComparer.Ordinal).Count() != all.Length ||
        ...
        {
            if (input[i] != output[i] &&
                !plan.Any(p => i >= p.Offset && i < p.Offset + indexSize))
            {
                throw new InvalidDataException(
                    "Unexpected byte change outside a TypeDef name.");
            }
        }
    ...
    public static Result RepairFile(string path, CapturedLayout? requiredLayout)
    {
        if ((File.GetAttributes(path) & FileAttributes.ReparsePoint) != 0)
            throw new IOException("Refusing a redirected target.");

        byte[] original = File.ReadAllBytes(path);
        var result = Prepare(original, requiredLayout);
        if (result.RenamedTypes == 0)
            return result;
        ...
        string backup = Path.Combine(
            backupDir,
            "UnityEngine.CoreModule." + Sha256(original) + ".original");

        if (File.Exists(backup))
        {
            if (Sha256(File.ReadAllBytes(backup)) != Sha256(original))
                throw new IOException("Existing backup differs.");
        }
        else
        {
        ...
        try
        {
            ...
            if (Sha256(File.ReadAllBytes(temporary)) != Sha256(result.Bytes) ||
                Sha256(File.ReadAllBytes(path)) != Sha256(original))
            {
